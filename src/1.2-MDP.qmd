...
The behavior of the agent over time is a **trajectory** (also called episode, history or roll-out) $\tau = (s_0, a_0, s_1, a_1, \ldots, s_T, a_T)$ defined by the dynamics of the MDP. Each transition occurs with a probability $p(s'|s, a)$ and provides a certain amount of reward defined by $r(s, a, s')$. In episodic tasks, the horizon $T$ is finite, while in continuing tasks $T$ is infinite.
...
![Example of a MDP with two actions in state $s_1$, leading to two different returns. The states $s_5$ and $s_6$ are terminal states, where no reward is received anymore.](img/return-example.svg){#fig-returnexample width=60%}
...
The value of taking an action $a$ in a state $s$ under the policy $\pi$ is the expected return when starting from that state, taking that action, and thereafter following $\pi$.
...
![Iterative policy evaluation. Source: David Silver. <http://www0.cs.ucl.ac.uk/staff/d.silver/web/Teaching.html>](img/iterativepolicyevaluation2.png){width=80%}
...
$$\max_a \; Q^{\pi} (s, a) = \max_a \, \sum_{s' \in \mathcal{S}} p(s' | s, a) \, [r(s, a, s') + \gamma \, V^{\pi}(s') ] \geq Q^\pi(s, \pi(s))$$
