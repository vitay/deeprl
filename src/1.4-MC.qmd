...
Evaluating a policy is only half of the job: what we want is to **improve** it. As seen with dynamic programming (@sec-dp), RL algorithms alternate between two steps, a principle called **generalized policy iteration**:

1. **Policy evaluation**: for the current policy $\pi$, estimate $V^\pi(s)$ or $Q^\pi(s, a)$. This is what the previous section did, by sampling episodes instead of solving the Bellman equations.
2. **Policy improvement**: from the current estimates, derive a new and **better** policy $\pi$.

After each episode, the values of the visited state-action pairs have changed, so the policy that was optimal w.r.t the old values might not be optimal anymore. As in dynamic programming, the policy can be improved **greedily**, by selecting in each state the action with the highest estimated value:

$$
    \pi'(s) = \text{argmax}_a \, Q(s, a)
$$

This is the moment where estimating **Q-values** rather than V-values pays off. With Q-values, improving the policy is immediate: one just looks at which action has the highest value. With V-values, one would have to look one step ahead to know where each action leads:

$$
    \pi'(s) = \text{argmax}_a \, \sum_{s' \in \mathcal{S}} p(s' | s, a) \, [r(s, a, s') + \gamma \, V(s')]
$$

which requires the dynamics $p(s' | s, a)$ and $r(s, a, s')$ of the MDP, i.e. exactly the model that we do not have. Estimating Q-values instead of V-values is what makes Monte Carlo methods **model-free**.

![Monte Carlo control alternates between policy evaluation by sampling and greedy policy improvement. See also @fig-gpi.](img/mc-control.png){#fig-mccontrol width=40%}

Putting the two steps together gives the **Monte Carlo control** algorithm:
...
5. For each state $s_t$ in the episode, **improve** the policy greedily:

$$
    \pi(s_t, a) = \begin{cases}
                    1\; \text{if} \; a = \text{argmax}_a \, Q(s_t, a) \\
                    0 \; \text{otherwise.} \\
                  \end{cases}
$$
...
This algorithm is correct, but as written it does not work. The reason is in step 5: the policy is **deterministic**, so in step 2 the agent will always take the same actions, always generate the same episode, and never find out whether the actions it never takes are better. The rest of this chapter is about fixing this.
...
1. An $\epsilon$-soft behavior policy over the **Q-values** as in on-policy MC is often enough, while a deterministic (greedy) policy can be learned implicitly.
...
A famous application of this second option is the control of the cooling system of Google's datacenters, which reduced their energy consumption by 40%. The RL algorithm did not control anything at first: it learned **passively** from the decisions taken by the existing controllers and the human operators, i.e. from a behavior policy, what the optimal policy should be.

::: {layout-ncol=2}
![The cooling installation of a Google datacenter.](img/googledatacenterpipes.0.jpg){#fig-datacenter-pipes}

![The resulting reduction in energy used for cooling. Source: <https://deepmind.com/blog/deepmind-ai-reduces-google-data-centre-cooling-bill-40/>](img/googledatacenter_1.png){#fig-datacenter-results}
:::

Learning from data collected by somebody else, without interacting with the environment, is called **learning from demonstrations** or **offline RL**, a topic to which we will come back in @sec-offlinerl. It is often the only realistic option: one does not experiment randomly with the cooling of a datacenter to see what happens.
...
To summarize what has been seen in this chapter:

* Monte Carlo methods estimate value functions by **sampling** complete episodes and averaging the returns obtained.

* They are **model-free**: by estimating Q-values rather than V-values, one can improve the policy without ever knowing $p(s' | s, a)$ and $r(s, a, s')$. The agent only needs to interact, i.e. trial and error.

* They only apply to **episodic tasks**. Since learning happens at the end of an episode, a continuing task, which never terminates, can simply not be learned. This is a severe restriction, and the main motivation for the temporal difference methods of the next chapter.

* They must solve the **exploration-exploitation** dilemma, in one of two ways: on-policy MC learns a stochastic policy ($\epsilon$-greedy, softmax) which explores by itself, while off-policy MC learns a greedy policy but explores through a separate behavior policy, at the price of importance sampling.

* Finally, and this is the property that will matter most in the rest of the book, MC estimates have a **small bias** but a **huge variance** (@sec-bandits). The bias is small because the returns $R_t$ are real, actually obtained returns: with enough episodes, the estimates converge to the true values, whatever the initial estimates were. The variance is huge because a single different action, or a single unlucky transition, can change the whole episode and therefore its return. This is why MC methods need so many samples to converge, i.e. why they have a bad **sample complexity**.

This last point is the fundamental trade-off of RL, and it explains the structure of everything that follows. Temporal difference methods (@sec-td) will accept a bias in order to reduce the variance, by replacing the real return by an estimate.
