...
![Successive frames in a video game are highly correlated.](img/breakout.png){#fig-breakout-correlated}
...
![The target network is used to compute the targets to train the value network. Its weights are regularly copied from the value network.](img/targetnetwork.png){#fig-targetnetwork}

![By keeping the targets constant for a while, the target network lets the value network catch up with them and converge optimally (in principle).](img/nonstationarity2.svg){#fig-nonstationarity2 width=80%}
