# Advantage Actor-Critic (A3C) {#sec-a3c}

## Actor-critic algorithms

The policy gradient theorem provides an actor-critic architecture that allow to estimate the PG from single transitions:
...
* $\psi_t = \sum_{k=0}^{n-1} \gamma^{k} \, r_{t+k+1} + \gamma^n \, V^\pi(s_{t+n}) - V^\pi(s_t)$ is the **n-step advantage actor-critic** (A2C).
...
2. For each state encountered, compute the discounted sum of the next $n$ rewards $\sum_{k=0}^{n-1} \gamma^{k} \, r_{t+k+1}$ and use the critic to estimate the value of the state encountered $n$ steps later $V_\varphi(s_{t+n})$.
