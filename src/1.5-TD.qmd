# Temporal Difference learning {#sec-td}
...
## Summary of TD methods

Before moving on, it is worth summarizing what temporal difference brings compared to the Monte Carlo methods of @sec-mc:

* TD learns value functions from **single transitions** $(s, a, r, s')$ instead of complete episodes. This is the main practical difference, and it has a major consequence: while MC only applies to episodic tasks, TD also works on **continuing tasks**, which never terminate.

* Like MC, TD methods are **model-free**: nothing about $p(s' | s, a)$ and $r(s, a, s')$ needs to be known, one only needs to sample transitions.

* Like DP, TD uses **bootstrapping**: an estimate is updated using other estimates. This is where its bias comes from, and also why it converges so much faster than MC.

* The **exploration-exploitation** dilemma is dealt with in the same two ways as in MC:

    * **On-policy** TD (SARSA) follows the learned stochastic policy, and learns the value of the action that was actually taken next:

    $$
        Q(s, a) \leftarrow Q(s, a) + \alpha \, (r(s, a, s') + \gamma \, Q(s', a') - Q(s, a))
    $$

    * **Off-policy** TD (Q-learning) follows a behavior policy but learns the value of the **greedy** action, i.e. of a deterministic policy that is not the one generating the data:

    $$
        Q(s, a) \leftarrow Q(s, a) + \alpha \, (r(s, a, s') + \gamma \, \max_{a'} Q(s', a') - Q(s, a))
    $$

Q-learning is the go-to method of tabular RL, and the rest of this book is to a large extent about making it work when the Q-values can no longer be stored in a table.
...
* The **forward view** solves the question left open by the n-step return, i.e. how to choose $n$, by deciding not to choose: all n-step returns are **averaged**, with a weight decreasing geometrically with $n$. This defines the $\lambda$-**return**:

$$
    R^\lambda_t = (1 - \lambda) \, \sum_{n=1}^\infty \lambda^{n-1} \, R^n_t
$$

The term $(1 - \lambda)$ is only there to normalize the weights, as the coefficients $\lambda^{n-1}$ sum to $\frac{1}{1-\lambda}$:

$$
    \sum_{n=1}^\infty \lambda^{n-1} = \dfrac{1}{1 - \lambda}
$$

so the weights $(1-\lambda) \lambda^{n-1}$ sum to one and the $\lambda$-return is a true weighted average of the n-step returns. Short returns (close to TD) get a high weight, long ones (close to MC) a small one, which is what we want: long returns have a high variance and should not dominate.

![The weights of the successive n-step returns in the $\lambda$-return decay geometrically. Source: @Sutton1998.](img/eligibility-forward.png){#fig-eligibilityweights width=60%}

To see what $\lambda$ does, one can split the infinite sum at the end of the episode at time $T$. All n-step returns with $n \geq T - t$ are simply the Monte Carlo return $R_t$, as there is nothing left to bootstrap from:

$$
    R^\lambda_t = (1 - \lambda) \, \sum_{n=1}^{T-t-1} \lambda^{n-1} \, R^n_t + \lambda^{T-t-1} \, R_t
$$

$\lambda$ therefore directly controls the bias-variance trade-off, exactly as $n$ did, but continuously:

* If $\lambda = 0$, all the weight goes to the first term and the $\lambda$-return is $R^1_t = r_{t+1} + \gamma \, V(s_{t+1})$, i.e. plain TD: high bias, low variance.
* If $\lambda = 1$, all the weight goes to the last term and the $\lambda$-return is the MC return $R_t = \sum_{k=0}^\infty \gamma^k \, r_{t+k+1}$: low bias, high variance.

Everything in between interpolates smoothly between the two, which is why TD($\lambda$) is usually written as a family of algorithms indexed by $\lambda$, with TD(0) and MC as its two extremes.

Subtracting the current estimate from the $\lambda$-return gives the $\lambda$-**advantage**, which takes a much simpler form:

$$
    A^\lambda_t = R^\lambda_t - V(s_t) = \sum_{k=0}^\infty (\gamma \lambda)^{k} \, \delta_{t+k}
$$

i.e. the discounted sum of all future TD errors. This is a remarkably convenient expression, as it only needs TD errors and no returns at all. We will meet it again under the name of **generalized advantage estimation** (@sec-GAE). Note that $\gamma$ and $\lambda$ seem to play a similar role in that formula, but they do not: $\gamma$ is also used inside each TD error $\delta_{t+k}$, so it defines the horizon of the value function itself, while $\lambda$ only weights how far back a prediction error is propagated.

The drawback of the forward view is visible in the formulas: the future transitions and their TD errors must be known when updating a transition, so the episode must be terminated before applying the updates, exactly as in MC. This prevents online learning.
...
$$
    e(s, a) = \begin{cases} e(s, a) + 1 \quad \text{if} \quad s=s_t \quad \text{and} \quad a=a_t \\
                            \gamma \, \lambda \, e(s, a) \quad \text{otherwise.}
              \end{cases}
$$
...
To summarize what eligibility traces bring:

* They are a very useful way to **speed learning up** in TD methods, by propagating a reward backwards over many transitions at once instead of one per episode.
* They give a continuous control over the bias-variance trade-off through $\lambda$, rather than the discrete choice of $n$.
* The modification applies to all TD methods: TD($\lambda$) for state values, SARSA($\lambda$) and Q($\lambda$) for action values.
* Their main drawback is **memory**: a trace has to be maintained for every state-action pair, not only the one just visited, and all of them have to be updated at each step. Clever implementations limit this by ignoring the traces that have decayed close to zero.
* The value of $\lambda$ has to be chosen carefully for each problem. In particular, when the initial actions of an episode are mostly random and found the reward by chance, a high $\lambda$ would reinforce them although they deserve no credit.
* Finally, if the problem is not strictly Markov, i.e. if it is a POMDP, eligibility traces can actually **help**, precisely because they update a whole history of transitions rather than a single state that does not contain all the necessary information.
...
$$
    A^{n}_t = \sum_{k=0}^{n-1} \gamma^{k} \, r_{t+k+1} + \gamma^n \, V^\pi(s_{t+n}) - V^\pi(s_t)
$$
...
Now, what is the optimal value of $n$? GAE decides not to choose and to simply average all n-step advantages, weighting them with a decaying parameter $\lambda$, exactly as the $\lambda$-return averaged the n-step returns. GAE is in fact nothing else than the $\lambda$-advantage introduced above, rediscovered in the context of deep RL. 

This defines the **Generalized Advantage Estimator** $A^{\text{GAE}(\gamma, \lambda)}_t$:

$$
    A^{\text{GAE}(\gamma, \lambda)}_t = (1-\lambda) \, \sum_{n=1}^\infty \lambda^{n-1} \, A^n_t = \sum_{l=0}^\infty (\gamma \lambda)^l \, \delta_{t+l}
$$

The GAE is simply a forward eligibility trace over distant n-step advantages: the 1-step advantage is more important than the 1000-step advantage (too much variance).

* When $\lambda=0$, we have $A^{\text{GAE}(\gamma, 0)}_t = A^{1}_t = \delta_t$, i.e. the TD advantage (high bias, low variance). 
